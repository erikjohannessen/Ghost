...
export default class App extends React.Component {
  ...
  async fetchLinkData(site, member) {
    ...
    if (qParams.get('action') === 'unsubscribe') {
      ...
      if (qParams.get('key') && qParams.get('uuid')) {
        ...
        // any malformed unsubscribe links should simply go to email prefs
        return {
          showPopup: true,
          page: 'accountEmail',
          pageData: {
    ...
    if (
      hasRecommendations({ site }) &&
      qParams.get('action') === 'signup' &&
      qParams.get('success') === 'true'
    ) {
  ...
  getContextPage({ site, page, member }) {
    ...
    if (!page || page === 'default') {
      const loggedOutPage =
        isInviteOnly({ site }) || !hasAvailablePrices({ site }) ? 'signin' : 'signup';
  ...
  transformPortalLinksToRelative() {
    document
      .querySelectorAll('a[href*="#/portal"], a[href*="#/share"]')
      .forEach(transformPortalAnchorToRelative);
  }
