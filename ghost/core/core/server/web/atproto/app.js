...
function requireAtproto(req, res, next) {
  if (!config.get('atproto:enabled')) {
    return next(
      new DisabledFeatureError({ message: 'ATProto login is not enabled on this site.' }),
    );
  }
  next();
}
  ...
  if (config.get('atproto:exclusive')) {
    ...
    return res.end(
      JSON.stringify({
        errors: [
          {
            message: 'This site requires sign-in via ATProto/Bluesky.',
