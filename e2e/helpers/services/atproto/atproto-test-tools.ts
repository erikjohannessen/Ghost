...
export async function insertAtprotoOAuthState(
  db: Knex,
  options: AtprotoStateOptions,
): Promise<string> {
...
async function writeAtprotoStateRow(
  db: Knex,
  values: Record<string, string | boolean | null>,
): Promise<void> {
  const database =
    db.client.config.connection && typeof db.client.config.connection === 'object'
  ...
  const result = spawnSync(
    'docker',
    ['exec', '-i', 'ghost-dev-mysql', 'mysql', '-uroot', '-proot', database],
    {
      input: `${sql}\n`,
      encoding: 'utf8',
    },
