...
import { AtprotoNeedsEmailPage, HomePage, SignInPage } from '@/helpers/pages';
import { MailPit } from '@/helpers/services/email/mail-pit';
import { extractMagicLink } from '@/helpers/services/email/utils';
import {
  createAtprotoTestDb,
  insertAtprotoOAuthState,
  MockAtprotoTokenServer,
} from '@/helpers/services/atproto/atproto-test-tools';
...
test.describe('Ghost Public - Portal ATProto', () => {
  ...
  function resolveKnexQuery<T>(query: PromiseLike<T>): Promise<T> {
    return Promise.resolve(query);
  }

  async function getCallbackRedirectLocation(
    ...
    if (!location) {
      ...
      throw new Error(
        `ATProto callback did not return a redirect location (status ${response.status()}): ${body}`,
      );
    }
  ...
  test('users can sign in with a Bluesky account when ATProto login is enabled', async ({
    ghostInstance,
    page,
  }) => {
    ...
    expect(
      await resolveKnexQuery(db('atproto_oauth_states').where({ id: stateId }).first()),
    ).toBeTruthy();
    const redirectLocation = await getCallbackRedirectLocation(
      page.request,
    ...
    const member = await resolveKnexQuery(db('members').where({ email }).first());
  ...
  test('updates the subscribed email when a linked Bluesky account signs in with a new email', async ({
    ghostInstance,
    page,
  }) => {
    const homePage = new HomePage(page);
    ...
    expect(
      await resolveKnexQuery(db('atproto_oauth_states').where({ id: firstStateId }).first()),
    ).toBeTruthy();
    const firstRedirectLocation = await getCallbackRedirectLocation(
      page.request,
      ghostInstance.baseUrl,
      firstStateId,
      'first-login',
    ...
    const initialMember = await resolveKnexQuery(
      db('members').where({ email: firstEmail }).first(),
    );
    expect(initialMember).toBeTruthy();
    expect(initialMember.atproto_did).toBe(did);

    const initialSubscriptionCount = Number(
      (
        await db('members_newsletters')
          .where({ member_id: initialMember.id })
    ...
    const secondRedirectLocation = await getCallbackRedirectLocation(
      page.request,
      ghostInstance.baseUrl,
      secondStateId,
      'second-login',
    ...
    const updatedMember = await resolveKnexQuery(
      db('members').where({ id: initialMember.id }).first(),
    );
    expect(updatedMember.email).toBe(secondEmail);
    expect(updatedMember.atproto_did).toBe(did);

    const updatedSubscriptionCount = Number(
      (
        await db('members_newsletters')
          .where({ member_id: initialMember.id })
          .count<{ count: string }>({ count: 'id' })
  ...
  test('prompts for an email when a first-time Bluesky account has none, then completes sign up after validation', async ({
    ghostInstance,
    page,
  }) => {
    const homePage = new HomePage(page);
    ...
    const member = await resolveKnexQuery(db('members').where({ email: submittedEmail }).first());
    expect(member).toBeTruthy();
    expect(member.atproto_did).toBe(did);
  ...
  test('does not prompt for an email again when a validated Bluesky member already exists', async ({
    ghostInstance,
    page,
  }) => {
    const homePage = new HomePage(page);
    ...
    const member = await resolveKnexQuery(db('members').where({ email: validatedEmail }).first());
    expect(member).toBeTruthy();
    expect(member.atproto_did).toBe(did);

    const subscriptionsBefore = Number(
      (
        await db('members_newsletters')
    ...
    const updatedMember = await resolveKnexQuery(db('members').where({ id: member.id }).first());
    expect(updatedMember.email).toBe(validatedEmail);
    expect(updatedMember.atproto_did).toBe(did);

    const subscriptionsAfter = Number(
      (
        await db('members_newsletters')
          .where({ member_id: member.id })
          .count<{ count: string }>({ count: 'id' })
