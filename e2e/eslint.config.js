...
const resetEnvironmentStaleFixtures = [
  'baseURL',
  'ghostAccountOwner',
  'page',
...
const noUnsafeResetEnvironment = {
  meta: {
    ...
    messages: {
      invalidLocation:
        'resetEnvironment() is only supported inside beforeEach hooks. Use a beforeEach hook or switch the file to usePerTestIsolation().',
      invalidFixtures:
        'Do not resolve {{fixtures}} in the same beforeEach hook as resetEnvironment(); those fixtures become stale after a recycle.',
    },
  },
  create(context) {
    return {
      CallExpression(node) {
        if (isBeforeEachHookCall(node)) {
          ...
          const staleFixtures = resetEnvironmentStaleFixtures.filter((fixtureName) =>
            fixtureNames.has(fixtureName),
          );
        ...
        const enclosingBeforeEachHook = [...ancestors]
          ...
          .find(
            (ancestor) =>
              isFunctionNode(ancestor) && ancestor.parent && isBeforeEachHookCall(ancestor.parent),
          );
...
export default tseslint.config([
  // Ignore patterns
  {
    ignores: ['build/**', 'data/**', 'playwright/**', 'playwright-report/**', 'test-results/**'],
  },
  ...
  // Keep assertions in test files and Playwright-specific helpers.
  {
    ...
    rules: {
      'no-restricted-syntax': [
        ...
        {
          selector:
            "ImportSpecifier[imported.name='expect'][parent.source.value='@playwright/test']",
          message: 'Keep Playwright expect assertions in test files.',
        },
        {
          selector:
            "ImportSpecifier[imported.name='expect'][parent.source.value='@/helpers/playwright']",
          message: 'Keep Playwright expect assertions in test files.',
        },
      ],
    },
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        'error',
        {
          ...
          message:
            'Use page objects or higher-level page methods instead of page.locator() in test files.',
        },
        {
          selector: 'MemberExpression[object.property.name="describe"][property.name="parallel"]',
          message:
            'test.describe.parallel() is deprecated. Use usePerTestIsolation() from @/helpers/playwright/isolation instead.',
        },
        {
          selector: 'MemberExpression[object.property.name="describe"][property.name="serial"]',
          message:
            'test.describe.serial() is deprecated. Use test.describe.configure({mode: "serial"}) if needed.',
        },
      ],
    },
  },
